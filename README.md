# Decks
My entry into [Google's Flutter Create contest](https://flutter.dev/create), the goal of which was to create a Flutter app
with only 5KB of Dart code. This entry manages to hit the max byte limit with the command
`find . -name "*.dart" | xargs cat | wc -c` showing exactly 5120 bytes.

The entire app fits in a single file with just about 200 lines.

This app is a small flashcard creator and viewer which persistently stores decks
and cards to local storage.

The app was designed to work best on Android but it should work just fine on iOS
as well.

It was specifically designed for small vertical layouts on phones.

## License

This Flutter application is licensed under the MIT license. Please read the [LICENSE](LICENSE) file for more information.
